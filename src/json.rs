...
impl Miner
{
    ...
    pub fn construct_merkle( &mut self, file_name: &str )
    {
        ...
        let nodes: Vec<Transaction> = Merkle::read_and_construct( file_name).unwrap();
 ...
 *
 * Content:
 *     - Contains the functionality for reading in the transaction log 
 *
 */

// Deserializes transactions into a vector so that the Merkle Tree can be easily constructed
#[derive( Serialize, Deserialize, Clone )]
pub struct Content
{

    // All posts are stored in a vector of off_blockway Transactions
    log: Vec<Transaction>
...
impl Content
{
    ...
    // Constructs the log from JSON
    #[allow(dead_code)]
    pub fn read_and_construct( file_name: &str ) -> Result< Content, Error >
    {

        // Construct the question 
        let string = Content::read_json( file_name ).unwrap();
        let content: Content = serde_json::from_str( string.as_ref() ).expect( "Failed to convert JSON to Question" );
        // Return the question
        Ok( content )
